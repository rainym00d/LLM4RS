    ...
    async def call_API(
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                append_to_jsonl([{"task_id": self.task_id}, self.request_json, self.result], save_filepath)
                status_tracker.num_tasks_in_progress -= 1
                status_tracker.num_tasks_failed += 1
        else:
            append_to_jsonl([{"task_id": self.task_id}, self.request_json, response], save_filepath)
            status_tracker.num_tasks_in_progress -= 1
